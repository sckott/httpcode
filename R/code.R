...
print_search <- function(text){
  ...
  if(any(found_codes)) print_codes(names(status_codes[found_codes])) else stopcode('No status code found for search: ', text)
}
...
http_code <- function(code = NULL){
  ...
  if(is.null(code)){
    ...
    if(is_three_digit_code(code)) print_code(code) else print_filtered_codes(code)
  }
}
